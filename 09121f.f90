module public_val
  ...
  double precision, parameter :: ymax = 0.005 ! y sizw
  ...
  integer, parameter :: my = 6 ! y grid num +2
  integer, parameter :: mmz = 120 ! z grid num, above
  integer, parameter :: mz = 172 ! z grid num +2, total
  integer, parameter :: xpnum = mx ! x key point num +2
  integer, parameter :: ypnum = my ! y key point num +2
  integer, parameter :: nxprocs = 10 ! num of subdomain
  ! time
  double precision, parameter :: dt = 5.0e-5 ! time step
  double precision, parameter :: tla = 180.0 ! time last
  ! fluid
  ...
  double precision, parameter :: fric = 0.5 ! tangential restitution coefficient
  double precision, parameter :: els1 = 0.9 ! normal restitution coefficient (mid-air collision)
  double precision, parameter :: fric1 = 0.5 ! tangential restitution coefficient (mid-air collision)
  double precision, parameter :: dpa = 250.0e-6 ! average particle diameter
  double precision, parameter :: dcgma = 50.0e-6 ! particle diameter standard deviation
  integer, parameter :: npdf = 2 ! bin num of particle distribution. must be odd number when iud=0
  double precision, parameter :: szpdf = dpa*10.0 ! thickness of the thin surface layer on particle bed
  ...
  double precision, parameter :: zbneg = dpa*1.0
  ! method
  ...
  ! if iud=0, normal distribution, mu=dpa, sigma=dcgma.
  ! if iud=1, d=dpa.
  ...
  integer, parameter :: nnkl = 40000 ! particle loc
  integer, parameter :: nnsf = 20000 ! surface, surfaced
  integer, parameter :: nnfx = 40000 ! flux 
  ! calculate after x steps
  integer, parameter :: tstart = 1 ! particle
  integer, parameter :: pstart = 1 !60000 ! surface
  ...
  double precision, dimension(xpdim, ypnum) :: pnch, pnch1, pnch2
  double precision, dimension(xpdim, ypnum, npdf) :: pdfch
  double precision, dimension(ypnum) :: eepnch1, eepnchr1
  double precision, dimension(ypnum) :: eepnch2, eepnchr2
  ...
  double precision, dimension(xpdim, ypnum) :: pz, pz0, pz1, pz2
  integer :: imtype
  ...
  double precision, dimension(nnpmax) :: cx, cy, cz
...
program main
  ...
  !if (mod(last, 100)==0) then
    ...
    write (100, "(5E15.5)") real(last), real(zkk), zb_now, htao(zkk)
    close(100)
  !end if
...
subroutine opfile
  ...
  if (myid==0) then
    ...
    write (32, *) 'variables = "XP", "YP", "ZP", "DP", "UP", "VP", "WP", "FK", "FZ"'
    close(32)
    !
    open (unit=33, file='./surface/surface0.plt')
    write (33, *) 'variables = "PX", "PY", "PZ", "PZ0", "PZ1", "PZ2"'
    close(33)
    !
...
subroutine grid
  ...
  double precision, dimension(mmz) :: zzdif
  !
  x = 0.0
  y = 0.0
  z = 0.0
  ...
  do k = 1, mmz
  zdif(k) = cc**(k-1)*zmax/((cc**mmz-1.0)/(cc-1.0))
  end do
  zzdif = zdif(1:mmz)
  minzdif = zdif(1)
  do k = mz-mmz, mz-1
  zdif(k) = zzdif(k-mz+mmz+1)
  end do
  do k = 1, mz-mmz-1
  zdif(k) = minzdif
  end do
...
subroutine imgd
  ...
  if (last==1 .or. ikl==0) then
    ...
    do j = 1, ypnum
    do i = 1, xpdim
    pz(i, j) = 6.0*dpa*sin((floor(time/10.0)+1.0)*16.0*pi*px(i)) + zb
    end do
    end do
    pz0 = pz
    pz1 = pz
    ...
    do j = 1, ypnum
    do i = 1, xpdim
    pnch(i, j) = pnch1(i, j) + pnch2(i, j)
    pz0(i, j) = pz0(i, j) + pnch(i, j)/aaa/por
    pz1(i, j) = pz1(i, j) + pnch1(i, j)/aaa/por
    pz2(i, j) = pz2(i, j) + pnch2(i, j)/aaa/por
    pz(i, j) = 6.0*dpa*sin((floor(time/10.0)+1.0)*16.0*pi*px(i)) + zb
    end do
    end do
    !do j = 1, ypnum
    ...
    !end do
    !end do
    call pxch(xpdim, ypnum, pz0, imtype, neighbor, comm3d)
    call pxch(xpdim, ypnum, pz1, imtype, neighbor, comm3d)
    call pxch(xpdim, ypnum, pz2, imtype, neighbor, comm3d)
    do i = 1, xpdim
    pz0(i, 1) = pz0(i, ypnum-1)
    pz1(i, 1) = pz1(i, ypnum-1)
    pz2(i, 1) = pz2(i, ypnum-1)
    pz0(i, ypnum) = pz0(i, 2)
    pz1(i, ypnum) = pz1(i, 2)
    pz2(i, ypnum) = pz2(i, 2)
    end do
    if (mod(last-100000,200001)==0.0) then
      pz0 = pz
      pz1 = pz
      pz2 = pz
    end if
...
subroutine imme
  ...
  include "mpif.h"
  integer :: i, j, k, n
  integer, dimension(nxdim+1, my+1, mz+1) :: fp
  integer :: flag1, flag2
  double precision, dimension(4) :: h
  double precision :: hh, vsol
  ! py
  !
  ! 3 -------------------       1 ------- 3 
  !   |\    |\    |\    |         |\    |   
  ...
  do j = 2, my-1
  do i = 2, nxdim-1
  flag1 = fp(i, j, k)+fp(i+1, j, k)+fp(i, j+1, k)+fp(i+1, j+1, k)
  if (flag1>0) then
    flag2 = fp(i, j, k+1)+fp(i+1, j, k+1)+fp(i, j+1, k+1)+fp(i+1, j+1, k+1)
    if (flag2>=4) then
      phirho(i, j, k) = phirho(i, j, k) - por - 1.0e-6
    else
      h(1) = pz(i, j) - zb_now - zw(k)
      h(2) = pz(i+1, j) - zb_now - zw(k)
      h(3) = pz(i, j+1) - zb_now - zw(k)
      h(4) = pz(i+1, j+1) - zb_now - zw(k)
      do n = 1, 4
      if (h(n)<0.0) h(n) = 0.0
      end do
      hh = sum(h)/4.0
      vsol = hh*xdif*ydif*por
      phirho(i, j, k) = phirho(i, j, k) - vsol/xdif/ydif/zdif(k)
...
subroutine cstf
  ...
  if (mod(last, nna)==0) then
    ...
    dudz = 0.0
    dudz = por/(1.0-por)*18.0/dpa**2*hru(1)*zdif(1)+dudz
    hru(2) = dudz*(z(2)-z(1)) + hru(1)
    mixl = kapa**2*lmd**2/nu/7.0*(hru(2)+hru(1))/2.0
    if (zkk-1>=3) then
      do k = 3, zkk-1
      dudz = (1.0-ahff(k))/ahff(k)*18.0/dpa**2*hru(k-1)*zdif(k-1)+dudz
      hru(k) = dudz*(z(k)-z(k-1)) + hru(k-1)
      end do
      mixl = kapa**2*lmd**2/nu/7.0*(hru(zkk-1)+hru(zkk-2))/2.0
    end if
    ddz = zdif(zkk-1)
    chru = hru(zkk-1)
    mixl = ntmixl(mixl, kapa, nu, chru, ddz)
    dudz = (sqrt(nu**2+4.0*mixl**2*htao(zkk)/rho)-nu)/2.0/mixl**2
    hru(zkk) = dudz*ddz + hru(zkk-1)
    shru = hru(zkk)/exp((z(zkk)-z(1))/lmd)
    if (abs(shru-hru(1))<1.0e-6) exit
    hru(1) = shru
    end do
    do k = zkk+1, mz
    ddz = zdif(k-1)
...
subroutine parstart
  ...
  !
  if (iud==0) then
    cgma = sqrt(log((dcgma*1.0e6)**2/(dpa*1.0e6)**2+1.0))
    mu = log(dpa*1.0e6) - 0.5*cgma**2
    tspdf = 0.
    do i = 1, npdf
    xx = dfloat(i-1)/dfloat(npdf-1)*dcgma*2.0 + (dpa-dcgma)
    xx = xx*1.0e6
    spdf(i) = 1.0/(sqrt(2.0*pi)*xx*cgma)*exp(-(log(xx)-mu)**2/(2.0*cgma**2))
    tspdf = tspdf + spdf(i)
    end do
    spdf = spdf/tspdf
  else if (iud==2) then
...
subroutine parcalculate
  ...
  integer :: npdie, nnpdie
  integer :: tnpdie(nxprocs)
  integer :: displs(nxprocs)
  ...
  double precision :: vch, vch1, vch2
  ...
  double precision :: ffz
  double precision :: mmp, nump
  double precision :: dzp
  ...
  double precision, dimension(xpdim, ypnum) :: tpdfch
  ...
  double precision, dimension(nspmax) :: tcx, tcy, tcz, tfk
  double precision, dimension(nspmax*20) :: ttcx, ttcy, ttcz, ttfk
  ...
  double precision, dimension(3) :: zdpa
  ...
  double precision, allocatable, dimension(:) :: chcx, chcy, chcz
  ...
  double precision, allocatable, dimension(:) :: chcxi, chcyi, chczi
  ...
  allocate(chcx(chmax), chcy(chmax), chcz(chmax))
  ...
  allocate(chcxi(chmax), chcyi(chmax), chczi(chmax))
  !
  ...
  ifimpact: if (fz(n)<=dpa*0.5) then
    ...
    mpin = mpin + mm1
    npin = npin + 1.0
    mupin(ip, jp) = mupin(ip, jp) + mm1*sqrt(upvec1(1)**2+upvec1(3)**2)
    mvpin(ip, jp) = mvpin(ip, jp) + mm1*sqrt(upvec1(2)**2+upvec1(3)**2)
    ...
    if (isp==0) then ! lammel
      ...
      if (vout(3)<sqrt(2.0*gg3*0.5*(d1+d2)) .or. pp<=0. .or. angout1<=0.0) then
        nne = 0
      else
        nne = 1
        if (pp>1.0) then
    ...
    if (nne==0) then
      ...
      vch1 = 0.0
      vch2 = 0.0
      !if (fk(n)>3.0*dpa .and. fk(n)<20.0*dpa) then
      if (fk(n)>3.0*dpa) then
        vch1 = vch
      else
        vch2 = vch
      end if
      pnch1(ii, jj) = pnch1(ii, jj) + vch1
      pnch2(ii, jj) = pnch2(ii, jj) + vch2
    else
      ammu2 = 0.0
      ...
      nnnp = nnnp + 1
      xp(nnnp) = point0(1) + zdpa(1) !+ upvec2(1)*dt
      yp(nnnp) = point0(2) + zdpa(2) !+ upvec2(2)*dt
      zp(nnnp) = point0(3) + zdpa(3) !+ upvec2(3)*dt
      up(nnnp) = upvec2(1)
      vp(nnnp) = upvec2(2)
      wp(nnnp) = upvec2(3)
      dp(nnnp) = d1
      ...
      cx(nnnp) = xp(nnnp)
      cy(nnnp) = yp(nnnp)
      cz(nnnp) = dfloat(myid) + 0.1
      wflx = wflx + nkl*mm1/xmax/ymax/dt
      vpout = vpout + upvec2
      norm_vpout = norm_vpout + norm_vout
      vvpout = vvpout + norm_vout**2
      mpout = mpout + mm1
      mupout(ip, jp) = mupout(ip, jp) + mm1*sqrt(upvec2(1)**2+upvec2(3)**2)
      mvpout(ip, jp) = mvpout(ip, jp) + mm1*sqrt(upvec2(2)**2+upvec2(3)**2)
      npout = npout + 1.0
      !
      ii = ipp
      jj = jpp
    ...
    if (ne>=1) then
      ...
      tempu(addnnp) = upvec2(1)
      tempv(addnnp) = upvec2(2)
      tempw(addnnp) = upvec2(3)
      tempd(addnnp) = dpd
  ...
  call MPI_ALLGATHER(npdie,1,inttype,tnpdie,1,inttype,comm3d,ierr)
  displs(1) = 0
  do i = 2, nxprocs
  displs(i) = displs(i-1) + tnpdie(i-1)
  end do
  call MPI_ALLGATHERV(tcx,npdie,realtype,ttcx,tnpdie,displs,realtype,comm3d,ierr)
  call MPI_ALLGATHERV(tcy,npdie,realtype,ttcy,tnpdie,displs,realtype,comm3d,ierr)
  call MPI_ALLGATHERV(tcz,npdie,realtype,ttcz,tnpdie,displs,realtype,comm3d,ierr)
  call MPI_ALLGATHERV(tfk,npdie,realtype,ttfk,tnpdie,displs,realtype,comm3d,ierr)
  call MPI_ALLREDUCE(npdie,nnpdie,1,inttype,MPI_SUM,comm3d,ierr)
  !if (last>10000) then
  !  do i = 1, nnpdie
  !  if (myid==0) then
  !    print*, ttcx(i)
  ...
  !end if
  do i = 1, nnpdie
  if (int(ttcz(i))==myid) then
    ii = int((ttcx(i)-px(1))/dpx) + 1
    jj = int((ttcy(i)-py(1))/dpy) + 1
    !print*, ii, xpdim-1
    if (ii<2) ii = 2
    if (jj<2) jj = 2
    if (ii>xpdim-1) ii = xpdim-1
    if (jj>ypnum-1) jj = ypnum-1
    vch = nkl*(pi*d1**3)/6.0
    vch1 = 0.0
    vch2 = 0.0
    if (ttfk(i)>3.0*dpa) then
      vch1 = vch
    else
      vch2 = vch
    end if
  ...
  if (addnnp>=1) then
    ...
    if (nnp>nnpmax) then
      ...
      cx(nnnp+1:nnnp+addnnp) = tempx(1:addnnp)
      cy(nnnp+1:nnnp+addnnp) = tempy(1:addnnp)
      cz(nnnp+1:nnnp+addnnp) = dfloat(myid) + 0.1
    end if
  else
    nnp = nnnp
  end if
  ...
  mp = rhos*volp
  ffz = fz(n) + zbneg
  call parloc(i, j, k, kk, xp(n), yp(n), ffz)
  ...
  ! up, xp development
  ffz = fz(n) + zbneg
  call parvol(up(n), vp(n), wp(n), xp(n), yp(n), ffz, dpd, mp, k, kk)
  fz(n) = ffz - zbneg
  zp(n) = fz(n) + point0(3)
  if (fz(n)>fk(n)) fk(n) = fz(n)
  !fk(n) = fk(n) + dt*up(n)
  end do
  !
  !aucreep = 0.0
  ...
  if (icol==1) then
    ...
    if (ffz<=0.0) then
      ...
      do k = 1, mz
      if (ffz>=zw(k) .and. ffz<zw(k+1)) then
        kk = k
        exit
      end if
      end do
    ...
    if (rrr<=rrrd .and. rrr>0.0) then
      ...
      beta1 = (2.0/7.0)*(1.0-fric1)/(1.0+eta1)
      alpha2 = (1.0+els1)/(1.0+eta2)
      beta2 = (2.0/7.0)*(1.0-fric1)/(1.0+eta2)
      !
  ...
  exch(9*nn+1:10*nn) = chcx(1:nn)
  exch(10*nn+1:11*nn) = chcy(1:nn)
  exch(11*nn+1:12*nn) = chcz(1:nn)
  ...
  if (nnn>0) then
    ...
    cx(nnp+1:nnp+nnn) = exchr(9*nnn+1:10*nnn)
    cy(nnp+1:nnp+nnn) = exchr(10*nnn+1:11*nnn)
    cz(nnp+1:nnp+nnn) = exchr(11*nnn+1:12*nnn)
    if (myid==0) then
      if (ikbx==0) then
        nnp = nnp + nnn
      end if
  ...
  exchi(9*nni+1:10*nni) = chcxi(1:nni)
  exchi(10*nni+1:11*nni) = chcyi(1:nni)
  exchi(11*nni+1:12*nni) = chczi(1:nni)
  ...
  if (nnni>0) then
    ...
    cx(nnp+1:nnp+nnni) = exchir(9*nnni+1:10*nnni)
    cy(nnp+1:nnp+nnni) = exchir(10*nnni+1:11*nnni)
    cz(nnp+1:nnp+nnni) = exchir(11*nnni+1:12*nnni)
    if (myid==dims-1) then
      if (ikbx==0) then
        nnp = nnp + nnni
      end if
  ...
  if (iud/=1) then
    ...
    tpdfch(i, j) = tpdfch(i, j) + pdfch(i, j, k)
    end do
    vlayer(i, j) = vlayer(i, j) + tpdfch(i, j)
    end do
    end do
    ! calculate pdf, zpdf
    do j = 2, ypnum-1
    ...
    do i = 1, xpdim
    if (iud==0) then
      dpaa(i, j) = dpaa(i, j) + pdf(i, j, k)*(dfloat(k-1)/dfloat(npdf-1)*dcgma*2.0+dpa-dcgma)
    else if (iud==2) then
      if (k==1) then
        dpaa(i, j) = dpaa(i, j) + pdf(i, j, k)*(dpa-dcgma)
      else if (k==2) then
        dpaa(i, j) = dpaa(i, j) + pdf(i, j, k)*(dpa+dcgma)
      else
        print*, 'npdf must = 2 while iud = 2'
        stop
      end if
    end if
    end do
...
subroutine parloc(iii, jjj, kkk1, kkk2, xxp, yyp, hhp)
  use public_val
  implicit none
  include "mpif.h"
  ! public
  integer :: iii, jjj, kkk1, kkk2
  double precision, intent(in) :: xxp, yyp, hhp
  ! local
  integer :: k
  !
  ...
  if (hhp<zw(2)) then
    kkk2 = 2
  else if (hhp>=zw(mz)) then
    kkk2 = mz
  else
    do k = 2, mz-1
    if (hhp>=zw(k) .and. hhp<zw(k+1)) then
      kkk2 = k
      exit
    end if
    end do
...
subroutine parvol(uup, vvp, wwp, xxp, yyp, hhp, ddp, mmp, k, kk)
  use public_val
  implicit none
  include "mpif.h"
  ! public
  ...
  double precision :: xxp, yyp, hhp
  double precision, intent(in) :: ddp, mmp
  ! local
  double precision :: alpha, beta
...
subroutine surfexch
  ...
  integer :: ierr
  integer :: status(MPI_STATUS_SIZE)
  double precision, dimension(ypnum) :: epnch1, epnchr1
  double precision, dimension(ypnum) :: epnch2, epnchr2
  double precision, dimension(ypnum) :: spnch1, spnchr1
  double precision, dimension(ypnum) :: spnch2, spnchr2
  ...
  !do j = 1, ypnum
  !epnch1(j) = pnch1(xpdim, j)
  !epnch2(j) = pnch2(xpdim, j)
  !spnch1(j) = pnch1(1, j)
  !spnch2(j) = pnch2(1, j)
  !do k = 1, npdf
  !jk = k + (j-1)*npdf
  ...
  !end do
  !!
  !call MPI_SENDRECV(eepnch1,ypnum,realtype,neighbor(2),107,  &
  !  eepnchr1,ypnum,realtype,neighbor(1),107,comm3d,status,ierr)
  !call MPI_SENDRECV(eepnch2,ypnum,realtype,neighbor(2),207,  &
  !  eepnchr2,ypnum,realtype,neighbor(1),207,comm3d,status,ierr)
  ...
  !!
  !call MPI_SENDRECV(epnch1,ypnum,realtype,neighbor(2),103,  &
  !  epnchr1,ypnum,realtype,neighbor(1),103,comm3d,status,ierr)
  !call MPI_SENDRECV(epnch2,ypnum,realtype,neighbor(2),203,  &
  !  epnchr2,ypnum,realtype,neighbor(1),203,comm3d,status,ierr)
  !call MPI_SENDRECV(epdfch,ypnum*npdf,realtype,neighbor(2),200,  &
  !  epdfchr,ypnum*npdf,realtype,neighbor(1),200,comm3d,status,ierr)
  !!
  !call MPI_SENDRECV(spnch1,ypnum,realtype,neighbor(1),104,  &
  !  spnchr1,ypnum,realtype,neighbor(2),104,comm3d,status,ierr)
  !call MPI_SENDRECV(spnch2,ypnum,realtype,neighbor(1),204,  &
  !  spnchr2,ypnum,realtype,neighbor(2),204,comm3d,status,ierr)
  !call MPI_SENDRECV(spdfch,ypnum*npdf,realtype,neighbor(1),108,  &
  !  spdfchr,ypnum*npdf,realtype,neighbor(2),108,comm3d,status,ierr)
  !!
  !do j = 1, ypnum
  !pnch1(3, j) = pnch1(3, j) + eepnchr1(j)
  !pnch2(3, j) = pnch2(3, j) + eepnchr2(j)
  !pnch1(2, j) = pnch1(2, j) + epnchr1(j)
  !pnch2(2, j) = pnch2(2, j) + epnchr1(j)
  !pnch1(xpdim-1, j) = pnch1(xpdim-1, j) + spnchr1(j)
  !pnch2(xpdim-1, j) = pnch2(xpdim-1, j) + spnchr2(j)
  !do k = 1, npdf
  !jk = k + (j-1)*npdf
  ...
  !do i = 1, xpdim
  !pnch1(i, 2) = pnch1(i, 2) + pnch1(i, ypnum)
  !pnch2(i, 2) = pnch2(i, 2) + pnch2(i, ypnum)
  !pnch1(i, ypnum-1) = pnch1(i, ypnum-1) + pnch1(i, 1)
  !pnch2(i, ypnum-1) = pnch2(i, ypnum-1) + pnch2(i, 1)
  !do k = 1, npdf
  ...
  call pxch(xpdim, ypnum, pnch1, imtype, neighbor, comm3d)
  call pxch(xpdim, ypnum, pnch2, imtype, neighbor, comm3d)
  ! y=2 send to y=ypnum, y=ypnum-1 send to y=1
  do i = 1, xpdim
  pnch1(i, ypnum) = pnch1(i, 2)
  pnch2(i, ypnum) = pnch2(i, 2)
  pnch1(i, 1) = pnch1(i, ypnum-1)
  pnch2(i, 1) = pnch2(i, ypnum-1)
  do k = 1, npdf
...
subroutine output
  ...
  double precision, dimension(xpnum, ypnum) :: tpz, tpz0, tpz1, tpz2, tpz4
  double precision, dimension((xpdim-2)*ypnum) :: apz, apz0, apz1, apz2
  double precision, dimension((xpnum-2)*ypnum) :: tapz, tapz0, tapz1, tapz2
  ...
  double precision, allocatable, dimension(:) :: txp, typ, tzp, tdp, tup, tvp, twp, tfk, tfz
  !
  nf = mod(last, nnf)
  ns = mod(last, nns)
  nc = mod(last, nnc)
  ...
  if (nf==0) then
    ...
    if (myid==0) then
      ...
      do j = 2, my-1
      do i = 2, mx-1
      write (42, "(5E15.5)") tx(i), y(j), z(k), tp(i, j, k)
      end do
      end do
      end do
      close(42)
  ...
  if (ns==0) then
    ...
    if (myid==0) then
      ...
      do k = 1, mz
      write (43, "(5E15.5)") z(k)-zbneg, thtao(k), thtaop(k), tahff(k), thru(k), apcoll(k)
      end do
      close(43)
    end if
    thtaop = 0.0
  ...
  if (ikl==1) then
    ...
    if (nc==0) then
      if (myid==0) then
        ...
        write (31, "(5E15.5)") time, real(tnnp)
        close(31)
      end if
    end if
  end if
    ...
    if (np==0) then
      if (last>=klstart) then
        ...
        if (myid==0) then
          ...
          do n = 1, tnnp
          write (32, "(5E15.5)") txp(n), typ(n), tzp(n), tdp(n), tup(n), tvp(n), &
            twp(n), tfk(n), tfz(n)
          end do
          close(32)
        end if
  ...
  if (nsf==0) then
    ...
    call MPI_ALLGATHER(apz0,numa,realtype,tapz0,numa,realtype,comm3d,ierr)
    call MPI_ALLGATHER(apz1,numa,realtype,tapz1,numa,realtype,comm3d,ierr)
    call MPI_ALLGATHER(apz2,numa,realtype,tapz2,numa,realtype,comm3d,ierr)
    ...
    if (myid==0) then
      ...
      do j = 2, ypnum-1
      do i = 2, xpnum-1
      write (33, "(5E15.5)") tpx(i), py(j), tpz(i, j), tpz0(i, j), tpz1(i, j), tpz2(i, j)
      end do
      end do
      close(33)
      !
      ...
      do j = 2, ypnum-1
      do i = 2, xpnum-1
      write (34, "(5E15.5)") tpx(i), py(j), tpz4(i, j)
      end do
      end do
      close(34)
    end if
  ...
  if (nfx==0) then
    ...
    if (myid==0) then
      ...
      if (twflx/=0.) then
        write (35, "(5E15.5)") time, tuflx, twflx, tuflx/twflx, tzbn
      else
        write (35, "(5E15.5)") time, tuflx, twflx, 0.0, tzbn
      end if
      close(35)
      !
      ...
      do k = 1, mz
      write (36, "(5E15.5)") z(k)-zbneg, tuflxz(k), twflxz(k)
      end do
      close(36)
      !
      open (unit=39, position='append', file='vin.plt')
      write (39, "(5E15.5)") time, tot_vpin(1), tot_vpin(2), tot_vpin(3), tot_nvpin
      close(39)
      !
      open (unit=46, position='append', file='vout.plt')
      write (46, "(5E15.5)") time, tot_vpout(1), tot_vpout(2), tot_vpout(3), tot_nvpout
      close(46)
      !
      open (unit=44, position='append', file='eminout.plt')
      write (44, "(5E15.5)") time, tot_vvpin, tot_vvpout, tot_mpin, tot_mpout
      close(44)
      !
      open (unit=45, position='append', file='numinout.plt')
      write (45, "(5E15.5)") time, tot_npin, tot_npout
      close(45)
    end if
    uflx = 0.0
    wflx = 0.0
  ...
  !
  deallocate(txp, typ, tzp, tdp, tup, tvp, twp, tfk, tfz)
...
function parsiz(ppp, dpa, dcgma, npdf)
  ...
  x = dcgma*2.*r1 + (dpa-dcgma)
  y = r2*rr
  xx = (x-dpa+dcgma)/dcgma/2.*dfloat(npdf-1)
  ii = int(xx) + 1
  if (ii>npdf) ii = npdf
  pdf = ppp(ii)
  if (n>10000) then
...
function arebound(alpha, beta, angin, dp2)
  ...
  double precision, parameter :: pi = acos(-1.0)
  !
  aa = beta/((alpha+beta)*sqrt(sqrt(3.0)*dp2*angin))
  bb = (9.0/2.0)*((1.0+alpha/beta)**2)*dp2*(3.0*sqrt(3.0)/4.0)
  xmin = -angin
  xmax = sqrt(bb*angin) - angin
  if (xmax>pi) then
    xmax = pi
  end if
  xmid = (xmax-xmin)/2.0 + xmin
  maa = (2.0/bb)*(1.0+xmid/angin)*log(bb*angin/((xmid+angin)**2))*1.1
  y = 1.0
  pdf = 0.0
  n = 0
  do
  ...
  x = r1*(xmax-xmin) + xmin
  y = r2
  pdf = (2.0/bb)*(1.0+x/angin)*log(bb*angin/((x+angin)**2))
  pdf = pdf/maa
  if (y<=pdf) exit
  end do
  arebound = x !+ angin
