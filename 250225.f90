module public_val
    ...
    real(kind=dbPc), parameter :: logMu = -8.1644 ! mu of lognormal distribution
    ...
    real(kind=dbPc), parameter :: binEnd = 7.0e-4 ! end of the particle diameter distribution
    ...
    real(kind=dbPc), parameter :: endTime = 600.0 ! The time that the simulation lasts
    ...
    do n = 1, pNum
        ...
        if (hp(n) <= 0.0 .and. wp(n) < 0.0) then
            ...
            if (whichDiameterDist == 1) then
                ...
                do nn = 1, npdf
                    currentHist(nn) = max(hist(ipp, jpp, nn), 0.0)
                end do
            ...
            if (v2z**2/(2.0*gHat) > zb) then
                remX = (sqrt((v2z/gHat)**2 - (2.0*zb)/gHat) + v2z/gHat)*v2x - 0.5*d2
            else
                remX = -1.0
            end if
            if (remX <= 0.0 .or. eBar <= 0.0) then
                rebound = .false.
            else
                rebound = .true.
            end if
            ...
            if (whichDiameterDist == 1) then
                ...
                d2 = 0.0
                do nn = 1, npdf
                    d2 = d2 + currentHist(nn)*currentDia(nn)**3
                end do
                d2 = d2**(1.0/3.0)
            end if
            m2 = (pi*d2**3)/6.0*rhoP
            ...
            Eeff = Ec*(1.0 - tau_f(1)/tau_s)
            Eeff = max(Eeff, 0.1*Ec)
            if (Ed1 > Eeff) then
...
!                            chunkDia(i, j, ks0) = chunkDia(i, j, ks0) + chunkHist(i, j, ks0, n)*currentDia(n)
!                        end do
!                        do k = ks0 + 1, ks
!                            chunkDia(i, j, k) = 0.0
!                            do n = 1, npdf
...
!                    dsf(i, j) = dsf(i, j) + tempHist(n)*currentDia(n)**3
!                end do
!                dsf(i, j) = dsf(i, j)**(1.0/3.0)
!
