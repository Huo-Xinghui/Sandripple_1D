...
def sample_averaged_eject(th, v1, gamma, d1, physical_dict, bed_type, dist_params):
    ...
    return Nej_mean, vn_mean

def get_model_data_array_v1(th, v1_array, gamma, d1_array, physical_dict, bed_type, dist_params):
    """Get model data array for Nej and vn vs v1."""
    Nej_list = []
    vn_list = []
    for d1, v1 in zip(d1_array, v1_array):
        if bed_type['monodisperse']:
            ...
            Nej, vn = sample_averaged_eject(th, v1, gamma, d1, physical_dict, bed_type, dist_params)
            Nej_list.append(Nej)
            vn_list.append(vn)
